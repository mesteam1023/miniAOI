...
# def template_check(source,image):
...
async def template_check(input_template, input_image):
    # img = cv.cvtColor(image, cv.IMREAD_GRAYSCALE)
    # img = cv.imread('Sources/partial_image.jpg', cv.IMREAD_GRAYSCALE)
    ...
    # template = cv.imread('Sources/partial_source_image.jpg', cv.IMREAD_GRAYSCALE)
    assert (
        input_template is not None
    ), "file could not be read, check with os.path.exists()"
    # w, h = input_template.shape[::-1]
    ...
    res = cv.matchTemplate(input_image, input_template, 5)
    min_val, max_val, min_loc, max_loc = cv.minMaxLoc(res)
    print(f'Max: {max_val}')
    print(f'Min: {min_val}')
    # If the method is TM_SQDIFF or TM_SQDIFF_NORMED, take minimum
